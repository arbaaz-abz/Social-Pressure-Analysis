...
def macro_model(request):
    if request.method == "POST":
        ...
        for filename in index_handle:
            ...
            user_month = month
            user_date = day
            # Finding all POI's discovered until the current date
            ...
            # For each POI a tuple is generated date|frequency|weight|related_terms
            print("Obtaining Analytics for each POI on : ", filename)
            for poi in pois:
                ...
                for tweet in handle:
                    if (
                        poi in tweet
                        or poi.capitalize() in tweet
                        or poi.upper() in tweet
                    ):
                        poi_frequency += 1
                handle.close()
            ...
            file_path = "trending_topics/visualization" + filename + ".html"
            count += 1
        return render(
            request,
            "trending_topics/macro_lda.html",
            {"topic_weights": topic_weights, "date": filename},
        )
    else:
...
@login_required
def micro_model(request):
    return render(request, "trending_topics/micro_lda.html")
